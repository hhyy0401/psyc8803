...
# ─── CV prediction ───
def cv_predict(X_df, y, model_name, params, ddm_col, top_n):

    splitter = KFold(n_splits=10, shuffle=True, random_state=42)
    y_pred = np.zeros(len(y))

    selected_per_fold = []  # (CHANGED) store selected features per fold
    ...
    for train_idx, test_idx in splitter.split(X_df):
        ...
        # (CHANGED) feature selection inside CV fold
        selected_features = select_top_features(corr, ddm_col, top_n)

        selected_per_fold.append(selected_features)  # (CHANGED)
    ...
    return y_pred, selected_per_fold  # (CHANGED)
...
def run_regression(X_df, y, model_name, ddm_col, top_n):

    params = find_best_params(X_df.values, y, model_name)

    # (CHANGED)
    y_pred, selected_per_fold = cv_predict(
        X_df,
        y,
        model_name,
        params,
    ...
    perm_p = permutation_test(X_df.values, y, model_name, params, r2)

    # (CHANGED) summarize features across folds
    flat_features = [f for fold in selected_per_fold for f in fold]

    freq = Counter(flat_features)

    top_features = [f for f, _ in freq.most_common(top_n)]
    ...
    return {
        ...
        "Features_Used": "; ".join(top_features),   # (CHANGED)
        "N_Features": len(top_features)             # (CHANGED)
    }
    ...
    for ddm_col in ddm_cols:
        ...
        for model_name in ALL_MODELS:
            ...
            row = {
                ...
                "N_Features": res["N_Features"],        # (CHANGED)
                "Features_Used": res["Features_Used"]   # (CHANGED)
            }
...
print(f"\n  {'Top_N':>5s}  {'Scenario':<15s} {'Target':<18s} {'Model':<12s} "
      f"{'R²':>8s} {'MAE':>8s} {'Perm_p':>8s}")

print(f"  {'─'*5}  {'─'*15} {'─'*18} {'─'*12} {'─'*8} {'─'*8} {'─'*8}")  # (CHANGED) removed extra column
